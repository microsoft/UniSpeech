...
from .conformer_encoder_layer import ConformerSentenceEncoderLayer


EXTRACTOR_MODE_CHOICES = ChoiceEnum(["default", "layer_norm"])
MASKING_DISTRIBUTION_CHOICES = ChoiceEnum(["static", "uniform", "normal", "poisson"])

def enc_streaming_mask(x_len, chunk_size, left_window=0):
    chunk_start_idx = torch.arange(0, x_len, chunk_size).long()
    ...
    if left_window != 0:
        ...
        mask = mask & mask_left
    result = torch.zeros(mask.size()).masked_fill(~mask, float("-inf"))
    return result
...
class Wav2Vec2Config(FairseqDataclass):
    ...
    structure_type: str = field(
        default="transformer", metadata={"help": "conformer or transformer"}
    )
...
class Wav2Vec2Model(BaseFairseqModel):
    def __init__(self, cfg: Wav2Vec2Config):
        ...
        if cfg.conv_type == "default":
            self.embed = feature_enc_layers[-1][0]
        elif cfg.conv_type == "conv2d":
            self.embed = feature_enc_layers[-1][0] * ((80 - 1)//2 - 1)//2
        else:
            self.embed = feature_enc_layers[-1][0] * 20
        ...
        if hasattr(cfg, 'streaming'):
            ...
            self.left_chunk = cfg.left_chunk
        else:
    ...
    def forward(
        ...
        if self.streaming:
            ...
            streaming_mask = enc_streaming_mask(seq_len, self.chunk_size,
                    left_window=self.left_chunk).to(features.device)
        else:
            streaming_mask = None
        ...
        x, layer_results = self.encoder(x, padding_mask=padding_mask, streaming_mask=streaming_mask, layer=None if output_layer is None else output_layer-1)
        ...
        results = {"features": x, "feautre_padding_mask": padding_mask}
...
class TransformerEncoder(nn.Module):
    def __init__(self, args):
        ...
        elif structure_type == "conformer":
            self.layers = nn.ModuleList(
                [
                    ConformerSentenceEncoderLayer(
                        embedding_dim=self.embedding_dim,
                        ffn_embedding_dim=args.encoder_ffn_embed_dim,
                        num_attention_heads=args.encoder_attention_heads,
                        dropout=self.dropout,
...
class TransformerSentenceEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.conformer_module = conformer_module

        if conformer_module:
            assert self.layer_norm_first
            self.in_fc1 = nn.Linear(self.embedding_dim, ffn_embedding_dim)
            self.in_fc2 = nn.Linear(ffn_embedding_dim, self.embedding_dim)
            self.cnn_module = ConvModule(self.embedding_dim,kernel_size = 3,dropout_rate = 0.1)
