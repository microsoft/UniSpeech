...
from fairseq import utils, checkpoint_utils, pdb
...
class Wav2Vec2Model(BaseFairseqModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument("--pretrained-path", help="path to pretrained model", default=None)
        parser.add_argument(
            "--extractor-mode",
    ...
    def __init__(self, args):
        ...
        if not self.transpose:
            ...
            self.q2h_proj = nn.Linear(final_dim, args.encoder_embed_dim)
        else:
            self.final_proj = nn.Linear(final_dim, args.encoder_embed_dim)
            self.q2h_proj = None
        ...
        if getattr(args, 'pretrained_path', None) is not None:
            ...
            for k, v in state.items():
                if 'w2v_encoder.w2v_model.' in k:
                    k = k.replace('w2v_encoder.w2v_model.', '')
                if k in model_state and model_state[k].size() == v.size():
                    new_state[k] = v
                    logging.info('load parameter {} in Wav2vec2 from path'.format(k, args.pretrained_path))
    ...
    def forward(self, source, padding_mask=None, mask=True, features_only=False):
        ...
        if self.quantizer:
            ...
            if self.negatives_from_everywhere:
                ...
                negs, _ = self.sample_negatives(negs, y.size(1))
        ...
        if not self.transpose:
            x = self.final_proj(x)
            results["q"] = self.q2h_proj(results["q"])
        else:
            y = self.final_proj(y)
...
@register_model_architecture("wav2vec2_s1", "wav2vec2_s1")
def base_architecture(args):
    ...
    args.transpose = getattr(args, "transpose", False)
