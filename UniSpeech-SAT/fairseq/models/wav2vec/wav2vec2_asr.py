...
from fairseq.models.rnnt_decoders import RNNTDecoder
...
class Wav2Vec2AsrConfig(FairseqDataclass):
    ...
    #data: Optional[str] = II("task.data")
...
class Wav2Vec2TransducerConfig(Wav2Vec2AsrConfig):
    ...
    encoder_proj_dim: int = field(
        default=512
    )
    joint_dim: int=field(
        default=512
    )
    embed_layer_norm: bool = field(
        default = True
    )
    embed_sigmoid: bool = field(
        default = True
    )
    linear_norm: bool = field(
        default=True
    )
...
class TransducerModel(FairseqEncoderDecoderModel):
    ...
    @classmethod
    def build_model(cls, cfg: Wav2Vec2TransducerConfig, task):
        """Build a new model instance."""
        ...
        return TransducerModel(encoder, decoder)
    ...
    def build_decoder(cls, cfg, tgt_dict):
        return RNNTDecoder(cfg, tgt_dict)
    ...
    def forward(self, **kwargs):
        enc_input = kwargs['net_input']
        encoder_out = self.encoder(tbc=False, **enc_input)
        decoder_out = self.decoder(encoder_out['encoder_out'], kwargs['target'])
        output = encoder_out
        output['decoder_out'] = decoder_out
        return output
...
class Wav2VecEncoder(FairseqEncoder):
    def __init__(self, cfg: Wav2Vec2AsrConfig, output_size=None):
        ...
        arg_overrides = {
            ...
            "left_chunk": cfg.left_chunk,
            "relative_position_embedding": cfg.relative_position_embedding,
        ...
        if getattr(cfg, "data", None) is not None:
            w2v_args.task.data = cfg.data
        if 'model' in w2v_args and not isinstance(w2v_args.model, omegaconf.dictconfig.DictConfig):
            model_conf = convert_namespace_to_omegaconf(w2v_args.model).model
        else:
            model_conf = w2v_args.model
        task = tasks.setup_task(w2v_args.task)
        model = task.build_model(model_conf)
        ...
        if output_size is not None:
            targ_d = output_size
        elif getattr(cfg, "encoder_proj_dim", d) != d:
            targ_d = cfg.encoder_proj_dim
