...
class MultiheadAttention(nn.Module):
    ...
    def __init__(
        ...
        if self.gru_rel_pos:
            self.gate_ur_linear = nn.Linear(self.q_head_dim, 8)
            self.eco_a = nn.Parameter(torch.ones(1, num_heads, 1, 1))
        ...
        if (
            ...
            if position_bias is not None:
                ...
                if self.gru_rel_pos:
                    ...
                    attn_mask_rel_pos = gate_u_1.view(bsz * self.num_heads, -1, 1) * position_bias
        ...
        if position_bias is not None:
            if self.gru_rel_pos == 1:
                ...
                gate_u, gate_r = torch.sigmoid(self.gate_ur_linear(query_layer).view(
                    _B, _H, _L, 2, 4).sum(-1, keepdim=False)).chunk(2, dim=-1)
                gate_u_1 = gate_u * (gate_r * self.eco_a - 1.0) + 2.0
                position_bias = gate_u_1.view(bsz * self.num_heads, tgt_len, 1) * position_bias
