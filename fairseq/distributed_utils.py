...
def infer_init_method(cfg: DistributedTrainingConfig, force_distributed=False):
    ...
    if all(
        ...
        cfg.distributed_init_method = "env://"  #tcp://" + os.environ["MASTER_ADDR"] + ":" + os.environ['MASTER_PORT']
        ...
        if node_list is not None:
            try:
                ...
                if ntasks_per_node == 1:
                    ...
                    # In the above example, local_rank will always be in [0, 1],
                    # which also matches torch.distributed.launch.
                    cfg.local_rank = local_id
                    ...
                    cfg.local_rank = int(os.environ.get("SLURM_LOCALID"))
    ...
    if cfg.pipeline_model_parallel:
        ...
        # if we have 4-way MP on a node with 8 GPUs, we want local_ranks to be 0
        # and 4, indicating the starting device IDs for each pipeline
        cfg.local_rank *= num_pipeline_devices
        ...
        if cfg.local_rank > 0:
            ...
            logger.debug(
                "setting CUDA device={} on rank {}".format(
                    cfg.local_rank, cfg.distributed_rank
                )
            )
            torch.cuda.set_device(cfg.local_rank)
            with open_dict(cfg):
                cfg.pipeline_devices = [cfg.local_rank + d for d in cfg.pipeline_devices]
            logger.info(
...
def distributed_init(cfg: FairseqConfig):
    ...
    if not cfg.common.tpu:
        ...
        cfg.distributed_training.local_rank = xm.get_local_ordinal()
...
def distributed_main(i, main, cfg: FairseqConfig, kwargs):
    cfg.distributed_training.local_rank = i
    if torch.cuda.is_available() and not cfg.common.cpu and not cfg.common.tpu:
        torch.cuda.set_device(cfg.distributed_training.local_rank)
...
def call_main(cfg: FairseqConfig, main, **kwargs):
    ...
    print("local_rank : {}".format(cfg.distributed_training.local_rank))
    if cfg.distributed_training.distributed_init_method is not None:
        ...
        if not cfg.distributed_training.distributed_no_spawn:
            ...
            distributed_main(cfg.distributed_training.local_rank, main, cfg, kwargs)
