...
class Wav2VecCtc(BaseFairseqModel):
    ...
    def __init__(self, w2v_encoder, args):
        ...
        self.args = args
        if getattr(args, 'w2v_path', None) is not None and not args.no_pretrained_weights:
            ...
            for k, v in state.items():
                if k in model_state and model_state[k].size() == v.size():
                    new_state[k] = v
                    logging.info('load parameter {} from pretrained model'.format(k))
...
class Wav2VecEncoder(FairseqEncoder):
    def __init__(self, args, tgt_dict=None):
        ...
        w2v_args.dropout = args.dropout
        w2v_args.activation_dropout = args.activation_dropout
        w2v_args.dropout_input = args.dropout_input
        w2v_args.attention_dropout = args.attention_dropout
        w2v_args.mask_length = args.mask_length
        w2v_args.mask_prob = args.mask_prob
        w2v_args.mask_selection = args.mask_selection
        w2v_args.mask_other = args.mask_other
        w2v_args.no_mask_overlap = args.no_mask_overlap
        w2v_args.mask_channel_length = args.mask_channel_length
        w2v_args.mask_channel_prob = args.mask_channel_prob
        w2v_args.mask_channel_selection = args.mask_channel_selection
        w2v_args.mask_channel_other = args.mask_channel_other
        w2v_args.no_mask_channel_overlap = args.no_mask_channel_overlap
        w2v_args.layerdrop = args.layerdrop
        w2v_args.feature_grad_mult = args.feature_grad_mult
        ...
        if hasattr(model, 'w2v_encoder'):
            model.w2v_encoder.w2v_model.remove_pretraining_modules()
        ...
        if hasattr(model, 'w2v_encoder'):
            self.encoder_dim = model.w2v_encoder.encoder_dim
            self.w2v_model = model.w2v_encoder.w2v_model
        else:
            self.encoder_dim = w2v_args.encoder_embed_dim
        ...
        if tgt_dict is not None:
            self.proj = Linear(self.encoder_dim, len(tgt_dict))
        elif getattr(args, "decoder_embed_dim", self.encoder_dim) != self.encoder_dim:
            self.proj = Linear(self.encoder_dim, args.decoder_embed_dim)
        else:
            self.proj = None
